...
func GetService(etcdAddr []string) *ServiceHub {
	hubOnce.Do(func() {
		if serviceHub == nil {
			...
			logger.HandelError("new etcd client error", err)
			serviceHub = &ServiceHub{
				client:        client,
				endPointCache: sync.Map{},
				watch:         sync.Map{},
...
func (s *ServiceHub) watchEndpoint(service string) {
	...
	logger.Log(fmt.Sprintf("watching %s node change ...", service))
	go func() {
		for resp := range ch {
