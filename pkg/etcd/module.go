package etcdregister

import (
	"context"
	"fmt"
	"time"

	"github.com/peterouob/seckill_service/pkg/config"
	etcdclient "github.com/peterouob/seckill_service/pkg/etcd/client"
	clientv3 "go.etcd.io/etcd/client/v3"
	"go.uber.org/fx"
)

var etcdClientHub = fx.Provide(
	func(cfg *config.Config) *etcdclient.ServiceHub {
		return etcdclient.GetService(cfg.EtcdConfig.Endpoints)
	},
)

var EtcdClientModule = fx.Module("etcd_client",
	fx.Provide(
		func(cfg *config.Config) (*clientv3.Client, error) {
			client, err := clientv3.New(clientv3.Config{
				Endpoints:   cfg.EtcdConfig.Endpoints,
				DialTimeout: 5 * time.Second,
			})
			if err != nil {
				return nil, fmt.Errorf("etcd client init failed: %w", err)
			}
			return client, nil
		},
	),
	fx.Options(etcdClientHub),
)

var EtcdServerModule = fx.Module("etcd",
	fx.Options(EtcdClientModule),
	fx.Provide(
		func(lc fx.Lifecycle, cfg *config.Config, client *clientv3.Client) *EtcdRegister {
			etcd := NewEtcdRegister(cfg.EtcdConfig.Endpoints, 5*time.Second.Milliseconds())
			lc.Append(fx.Hook{
				OnStart: func(ctx context.Context) error {
					return etcd.Register(cfg.EtcdConfig.ServiceName, cfg.GrpcAddr)
				},
				OnStop: func(ctx context.Context) error {
					return etcd.UnRegister(cfg.EtcdConfig.ServiceName, cfg.GrpcAddr)
				},
			})
			return etcd
		},
	))
