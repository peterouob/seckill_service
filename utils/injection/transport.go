...
func ProvideGrpcServer(lc fx.Lifecycle, p GrpcServerParams) (*grpc.Server, GrpcServerReady) {
	...
	grpcAddr := utils.FormatIP(p.Config.GrpcAddr)
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			lis, err := net.Listen("tcp", grpcAddr)
			if err != nil {
				logs.Error("grpc listen failed", err)
				return err
			}

			healthSrv.SetServingStatus("", grpc_health_v1.HealthCheckResponse_SERVING)
			go func() {
				logs.Logf("gRPC server listening on %s", grpcAddr)
				close(readyC)
		...
		OnStop: func(ctx context.Context) error {
			logs.Log("gRPC server graceful stopping...")
			...
			select {
			case <-stopC:
				logs.Log("gRPC server stopped gracefully")
				return nil
			case <-ctx.Done():
				logs.Log("gRPC graceful stop timeout, forcing stop")
				server.Stop()
				return nil
			}
		},
...
func ProvideHTTPServer(lc fx.Lifecycle, p HTTPServerParams) *gin.Engine {
	...
	httpAddr := utils.FormatIP(p.Config.HttpAddr)
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			go func() {
				logs.Logf("HTTP server listening on %s", httpAddr)
				serverErrors <- srv.ListenAndServe()
			}()
			return nil
		},
		OnStop: func(ctx context.Context) error {
			logs.Log("HTTP server graceful stopping...")
			if err := srv.Shutdown(ctx); err != nil {
				logs.Error("HTTP server shutdown error: %v", err)
				return srv.Close()
			}
			logs.Log("HTTP server stopped gracefully")
			return nil
		},
	})
	return server
